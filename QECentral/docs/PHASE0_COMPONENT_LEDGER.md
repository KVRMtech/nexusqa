...
- **(a) crawl-recorder core — DONE + tested (pure).** `login_recorder.recipe_from_observed_login(observation)` → `{steps, slots, login_type_key}` from one crawler observation; record-time key == match-time key (verified). Test `test_login_recorder.py`.
- **(a)(b) API endpoints — DONE (import + route-registration verified; runtime awaits deploy).** `POST .../recipes/from-observation` (observed login → `recipe_from_observed_login` → `save_recipe` with the key, Editor+, audited) and `POST /recipes/reuse-check` (login form → key → `find_recipes_by_login_type` → {reuse|record}). Router imports clean, both routes registered (137 total).
- **STILL NOT done (need live-crawl or the coordinated backend deploy):**
  - **(a) qe-explorer must EMIT the observation** (login_path/fields/submit/verify-documents/landing) in the crawl-completion payload, and `complete_crawl` (qe-central) must relay it to `POST .../recipes/from-observation`. This is a crawler change — only truly verifiable by a live crawl; deliberately not done unattended in this fragile zone.
  - **(b) frontend reuse-prompt UI** in onboarding (calls `/recipes/reuse-check`) — buildable locally but its value depends on the endpoint being deployed.
  - **Runtime verification** of the two endpoints (HTTP+DB) — awaits the coordinated **backend deploy** (platform-api rebuild + apply `apply_login_type_key.sql`; deferred: fragile zone + one outage already this session).
  - **(c) `/members` API aliases** — low value (no caller; the UI rename is what users see, and it's LIVE). Skipped.
  - **The E2E crawl of VKPower Life** — the finale, after the backend deploy.
  Branch `feat/record-once-run-anywhere` (18 commits).
