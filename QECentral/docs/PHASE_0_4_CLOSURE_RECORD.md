# Phase 0-4 â€” Closure Record With Findings
...
named below, and this document changes nothing about that refusal â€” it is
quoted verbatim in Â§3. The original Phase 0-4 exit criteria (each gate's own
document, cited in Â§3) were **not met**.
...
or closing Phase 0-4 on the evidence actually in hand â€” that Phase 5 may open
...
## Â§1 Â· What is verified, and by whom

Every fact in this section was independently checked by the certifying agent
session (`nexusqa-ae`) â€” not accepted on a report â€” using the method noted
beside it.

### 1.1 Â· The one admissible crossing

| | |
| --- | --- |
| Application | `acme-life` |
| Bundle | `Nexus_power/evidence/gate2/r3_acme_reproduced_7d7408b/journey.json` |
| Verdict | `crossed: 2 ['Bind policy', 'Bind policy']` Â· `confirmation observed: True ['dialog']` |
| `produced_by` | head `7d7408b`, **dirty: false** |
| Admissibility | `t3_verify_crossing_evidence.py --sha 7d7408b` â†’ **[ADMISSIBLE]**, admissible 1/1, exit 0 â€” re-run by `nexusqa-ae` against the artifact directly, not the paste |
...
### 1.2 Â· The deployment

| | |
| --- | --- |
| Environment | `verdict-box`, GCP `project-8d85a07a-396c-40aa-9b6`, `asia-southeast1-a` |
| VM `HEAD` | `7d7408bf0ca359a1ccec9a093523dc74b9dc37a9` â€” confirmed live over SSH by `nexusqa-ae`: `git rev-parse HEAD` exact match, `git status --short` empty |
| Containers | `nexus-qe-central`/`nexus-qe-explorer` healthy; created `2026-08-27T02:46:52Z` â€” **not** rebuilt during the fast-forward from `3d7e0f5` to `7d7408b`, which is sound only because `git diff --stat be20767..7d7408b` (run independently) touches exactly one file, `tests/browser/test_gate2_three_applications.py`, and zero service files |
...
| Schema | `qec_023`, per `Nexus_power/evidence/r5_deployed_service/deployed_service.json` (captured before the fast-forward) â€” **not independently re-queried after** the fast-forward, but no migration ran between the two states, only a git checkout, so it is not expected to have changed |
| Golden crawl gate | `GATE_VERDICT=PASS`, exploration `c57abb74-cae3-4efc-92a8-a5b7328c2326`, 11 ratchet metrics **RISE** (new floors: pages 22â†’30, forms 7â†’8, submitted 9â†’13, flows 7â†’12, catalog_questions 67â†’80, five more), 0 regressions. Artifact: `Nexus_power/evidence/r5_golden_gate/golden_gate_output.txt`. Exploration row independently re-queried by `nexusqa-ae` directly against the VM's own database this turn (`docker exec nexus-postgres psql -U nexus -d qecentral`), not taken from the artifact's own claim: `c57abb74-cae3-4efc-92a8-a5b7328c2326\|completed`. |
...
**service code**, which the `be20767..7d7408b` delta does not touch (Â§1.2
...
correct call at the time it was made, not a mistake corrected â€” the
evidence did not yet exist on disk.

### 1.3 Â· Two applications, driven to their boundaries, gaps measured and named

Full record: `QECentral/docs/PHASE_1_EXIT_RESCOPE.md` (architect, 2026-08-24).

* **summit-life-carrier** â€” crossed 1 (`Submit Application`), **refused** by
...
* **vkpower-life** â€” driven to `/apply/beneficiary/` (the payment step's
...
| ID | Gap |
| --- | --- |
| B1 | DOM-diff rejection reader â€” read a plain-text rejection with no accessibility annotation |
| B2 | Constraint-aware repair â€” generate values that satisfy declared application constraints, not just widget shape |
| B3 | List sub-action / allocation-rule / cross-step coherence (vkpower's two remaining stalls) |
| B4 | Seed targeting and ledger completeness â€” near-duplicate field labels hash apart; five wizard fields absent from the field ledger entirely |

### 1.4 Â· Four accepted-with-findings items

Full record: `QECentral/docs/CEREMONY_READOUT_PACK.md` Â§2.

1. **Lexical-gate residual** â€” `Phase.WALK`'s mutation gate is
...
2. **A11e cross-interpreter matrix** â€” advisory by design; its separate
...
3. **Phase-1 exit re-scope** â€” Â§1.3 above; the sentence to keep saying
...
* Not claimed: that A25 (Gate 3, "Phase 2 is deployed") is closed. The
  document's stated blocker â€” "18-Aug build, 108 commits behind" â€” is
  stale; the VM is now at `7d7408b`/`qec_023` (Â§1.2). But the M2.1 browser
  proof that would actually close A25 has not been run â€” the VM host has
  neither `pytest` nor the right container has both `pytest` and
  Playwright together â€” so A25 is reported here as **blocked on tooling
...
* Not claimed: that `a37_2` (PAT revocation) is confirmed in a form this
  ...
  this record holds the same standard as the Gate 5 ceremony record â€” a
  security attestation needs to reach the certifying session directly, not
  by relay, however faithfully relayed.

---

## Â§3 Â· The Gate 5 refusal this record preserves, not replaces
...
## Â§4 Â· What this record authorises, and only on signature
...
authorises **Phase 5 entry** on the basis stated in Â§1, with the gaps of
Â§1.3/Â§1.4 and the refusal of Â§3 carried forward as open, named work â€”
...
*This line is intentionally empty. No agent session â€” this one or any
peer â€” writes a name into it. `GATE_5_CEREMONY.md`'s rule applies here
...
"A". Every fact in Â§1 was checked independently by `nexusqa-ae` before being
written here â€” SSH to the VM, a direct re-run of `t3_verify_crossing_evidence.py`,
a direct re-run of `gate5_verify_ceremony.py`, and direct reads of every gate
document cited in Â§2 â€” none of it taken from the peer's or the owner's word
alone. Written at repository HEAD `7f641a0`, working tree clean apart from
this new file; Â§1.2's golden-gate row added in a follow-up edit at HEAD
