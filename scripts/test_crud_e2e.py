...
def main():
    ...
    payload = {
        ...
        "data_workbook": [
            {"field_name": "FirstName", "field_value": "Sarah", "field_type": "string"},
            {"field_name": "Email", "field_value": "sarah@test.com", "field_type": "email"},
        ],
    }
    ...
    # 3) List test cases
    r = httpx.get(f"{BASE}/api/v1/test-cases", params={"tenant_id": "nexus-platform"}, headers=hdr, timeout=15)
    ...
    # 6) Stats
    r = httpx.get(f"{BASE}/api/v1/test-cases/stats", params={"tenant_id": "nexus-platform"}, headers=hdr, timeout=15)
