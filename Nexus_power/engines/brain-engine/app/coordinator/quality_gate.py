...
class QualityGate:
    ...
    def evaluate_canonical(
        ...
        scene_qualities: list[str] | None = None,
    ) -> dict[str, Any]:
        """
    ...
    def _score_visual_quality(
        ...
        if not has_visual and scene_count == 0:
            # No visual data extracted — visual analysis failed or was skipped.
            # Return 0.0 so the weighted overall score falls below the pass threshold
            # when a video upload was expected to produce visual evidence.
            return 0.0
        ...
        # ── Per-scene quality distribution (eyes engine self-assessment) ─
        # build_scenes() flags each scene as 'strong' / 'degraded' / 'weak'
        # based on how many independent signals supported the screen's
        # identity. Without weighing this in, the gate scores "excellent"
        # on artifacts where every scene is flagged weak — that is the
        # specific contradiction observed on 3793cf9a (gate=1.00, 9/12
        # scenes weak/degraded). Weight strong=1.0, degraded=0.6, weak=0.25.
        if scene_qualities:
            n = len(scene_qualities)
            if n > 0:
                weights = {"strong": 1.0, "degraded": 0.6, "weak": 0.25}
                weighted = sum(weights.get((q or "").lower(), 0.5) for q in scene_qualities)
                quality_factor = weighted / n
                strong_frac = sum(1 for q in scene_qualities if (q or "").lower() == "strong") / n
                weak_frac = sum(1 for q in scene_qualities if (q or "").lower() == "weak") / n
                if quality_factor < 0.4:
                    warnings.append(
                        f"Per-scene quality is poor ({weak_frac:.0%} weak, "
                        f"{strong_frac:.0%} strong) — eyes engine flagged "
                        "most scenes as unreliable; do not trust visual evidence"
                    )
                elif quality_factor < 0.6:
                    warnings.append(
                        f"Per-scene quality is mixed ({weak_frac:.0%} weak, "
                        f"{strong_frac:.0%} strong) — partial visual reliability"
                    )
                # Pull the score down toward quality_factor — a video where
                # every scene is weak should never score above ~0.5 on
                # visual_quality regardless of how many scenes or how long
                # the descriptions are.
                score = min(score, max(quality_factor, 0.1))
