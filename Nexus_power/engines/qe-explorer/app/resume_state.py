...
def build_checkpoint(
    *, frontier: Sequence[Any], visited: Iterable[str], states: int, actions: int,
    ...
    journeys_walked: int = 0, journey_crossings: int = 0,
) -> dict:
    ...
    return {
        ...
        # ── Gate 1 / T-JC-02 · PROGRESSION SURVIVES A RESUME ─────────────────
        # ``adjudicate`` refuses a completion claim from a run that walked
        # journeys and crossed no step. A resumed run re-walks flows it may have
        # no new progress to make in - its predecessor already crossed the funnel
        # - and judging it on its OWN crossings alone would refuse every
        # late-stage resume of a crawl that had already succeeded. Same doctrine
        # as ``states``: the evidence belongs to the CRAWL ID, not to the run.
        #
        # UNDER-COUNTS BY DESIGN, and only in the safe direction. A checkpoint is
        # periodic, so a crossing made after the last one is not inherited. That
        # can cause a conservative refusal; it can never manufacture a
        # progression that did not happen, which is the only direction a machine
        # that may exclusively pull verdicts DOWN is allowed to be wrong in.
        "journeys_walked": _int(journeys_walked),
        "journey_crossings": _int(journey_crossings),
...
class ResumePlan:
    ...
    prior_actions: int = 0
    #: The durable CROSSING JOURNAL (M3.4 / T-RS-01): every irreversible
    #: boundary this crawl id has already reserved or crossed, in manifest
    #: order.  Replayed into :meth:`app.boundary.CrossingLedger.restore` so a
    #: resumed crawl refuses a boundary the killed one already spent.  Carried
    #: SEPARATELY from the checkpoint because a checkpoint is periodic and an
    #: irreversible action cannot wait for the next period - these records are
    #: written ahead of the click, so every one of them counts, including the
    #: ones written after the last checkpoint.
    crossings: tuple = ()
    #: Journeys and FORWARD STEP TRANSITIONS already durable under this crawl id,
    #: read from the newest checkpoint (Gate 1 / T-JC-02).  Zero when the prefix
    #: predates this field, which is exactly the pre-Gate-1 behaviour.
    prior_journeys: int = 0
    prior_crossings: int = 0
    ...
    def as_dict(self) -> dict:
        return {"frontier": len(self.frontier), "spent_keys": len(self.spent_keys),
                ...
                "prior_journeys": self.prior_journeys,
                "prior_crossings": self.prior_crossings,
...
def rebuild(records: Iterable[Mapping[str, Any]], *, resuming: bool) -> ResumePlan:
    ...
    """
    from .emit import REC_ACTION, REC_CROSSING, REC_PAGE_STATE  # local: keeps this module leaf-ish
    ...
    for rec in records or ():
        ...
        if rtype == REC_PAGE_STATE:
            ...
            prior_actions += 1
        elif rtype == REC_CROSSING:
            # EVERY crossing record, not the last one.  Unlike a checkpoint
            # (a snapshot, where the newest is the whole truth) the journal is
            # an APPEND-ONLY LOG of distinct irreversible acts, and dropping any
            # entry re-authorises exactly the boundary it described.  Both the
            # `reserved` and the later `crossed` record for one crossing are
            # kept; they carry the same boundary key, and restore folds them
            # into one spend.
            crossings.append(rec)
        elif rtype == REC_CHECKPOINT:
            checkpoint = rec          # last one wins: a checkpoint is a snapshot

    # A CROSSING COUNTS AS A PREFIX.  The guard below decides whether this crawl
    # id has durable evidence to continue from, and a journal entry is the most
    # consequential evidence there is: a crawl killed after reserving a boundary
    # but before its first page_state has mutated the application exactly once.
    # Treating that as "no prefix" would drop the journal on the floor and let
    # the resumed crawl cross the same boundary again - the precise duplicate
    # this milestone exists to prevent, reintroduced by the emptiness check.
    if prior_states == 0 and checkpoint is None and not crossings:
        return ResumePlan(
    ...
    return ResumePlan(
        ...
        # From the checkpoint, not counted from records: a step transition has no
        # record type of its own, and the checkpoint is where the walker's own
        # tally is already persisted. `.get` with 0 keeps a pre-Gate-1 manifest
        # readable rather than refusing it.
        prior_journeys=_int((checkpoint or {}).get("journeys_walked")),
        prior_crossings=_int((checkpoint or {}).get("journey_crossings")),
