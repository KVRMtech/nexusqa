...
class LegsConfig(EngineConfig):
    ...
    # Working directory for Playwright artifacts (screenshots, HAR logs,
    # traces). Treat as ephemeral; durable copies go to the artifact store.
    evidence_storage_path: str = "./data/evidence"
    evidence_delete_after_upload: bool = Field(
        default=True,
        validation_alias="LEGS_EVIDENCE_DELETE_AFTER_UPLOAD",
    )
...
class LegsEngine(NexusEngine, GPUWorkerMixin):
    def __init__(self):
        self.cfg = LegsConfig()
        super().__init__(
            name="legs",
    ...
    async def on_startup(self):
        ...
        logger.info(
            "legs.storage_backend backend=%s working_dir=%s delete_after_upload=%s",
            self._artifacts.backend_name,
            self.cfg.evidence_storage_path,
            self.cfg.evidence_delete_after_upload,
        )
        ...
        # ── Durable Job Queue (Redis Streams) ─────────────────
        ...
        if self.is_worker_mode and self._job_queue and self._job_queue.is_connected:
            ...
            logger.info(
                "legs.worker_loop_started",
                extra={"mode": self.engine_mode},
            )
    ...
    async def _process_queued_job(self, job: dict):
        ...
        action = payload.get("action", "run_test")
        if action == "run_exploration":
            ...
            tc = TestCase(**payload["test_case"])
            await self._run_test(
                job_id=payload["job_id"],
                test_case=tc,
                target_type=payload.get("target_type", "api"),
    ...
    async def _run_test(
        ...
        try:
            ...
            await self._sync_evidence_to_artifact_store(
                tenant_id=tenant_id,
                job_id=job_id,
                local_dir=evidence_dir,
                namespace_segment=job_id,
            )

    async def _sync_evidence_to_artifact_store(
        self,
        tenant_id: str,
        job_id: str,
        local_dir: str,
        ...
        """Upload evidence dir contents to object storage, then GC locally."""
        if not local_dir or not os.path.isdir(local_dir):
            return
        try:
            key_prefix = self._artifacts.build_key(
                tenant_id, "legs", namespace_segment,
            )
            refs = await self._artifacts.upload_directory(
                key_prefix, local_dir,
            )
            logger.info(
                "legs.evidence_uploaded job_id=%s count=%d backend=%s",
                job_id, len(refs), self._artifacts.backend_name,
            )
        except Exception as e:
            logger.warning(
                "legs.evidence_upload_failed job_id=%s error=%s",
                job_id, str(e),
            )
            return
    ...
    async def _run_exploration(
        ...
        try:
            ...
            await self._sync_evidence_to_artifact_store(
                ...
                job_id=job_id,
                local_dir=evidence_dir,
                namespace_segment=f"explore_{job_id}",
            )
