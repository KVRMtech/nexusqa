...
AuthStore uses PostgreSQL as primary storage. Optional in-memory fallback is
disabled by default so E2E and operator testing fail fast when the database is
...
class AuthStore:
    """Dual-backend storage: PostgreSQL primary, opt-in in-memory fallback."""

    def __init__(self):
        self._db: Optional[Database] = None
        self._allow_memory_fallback = (
            os.getenv("AUTH_ALLOW_IN_MEMORY_FALLBACK", "false").lower() == "true"
        )
    ...
    async def connect(self, pg_config) -> None:
        """Attempt PostgreSQL connection. Raise by default on failure."""
        try:
            ...
            if not self._allow_memory_fallback:
                raise RuntimeError(
                    "Auth service requires PostgreSQL. "
                    "Set AUTH_ALLOW_IN_MEMORY_FALLBACK=true only for explicit local dev fallback."
                ) from exc
