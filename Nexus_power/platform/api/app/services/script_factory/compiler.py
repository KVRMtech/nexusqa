...
def _ladder(observed: dict, kind: str) -> str:
    ...
    label = js_str(observed.get("label", ""))
    anchor = js_str(observed.get("anchor", ""))
    scope = f"page.getByRole('row', {{ name: '{anchor}' }})" if anchor else "page"
...
def _action_lines(step, field_meta: dict) -> list[str]:
    ...
    if verb == "navigate":
        ...
        if note:
            out.append(note)
        if kind == "select":
            out.append(f"await {_ladder(observed, 'select')}.selectOption('{js_str(value)}');")
        elif kind == "date":
            out.append(f"await {_ladder(observed, 'date')}.fill('{js_str(value)}');")
            out.append(
