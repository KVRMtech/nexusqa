...
class RunConfigRequest(BaseModel):
    ...
    # Team E: completed journeys compile from their own crawl evidence and may
    # run without an adopted artifact case.  These are PAYLOADS, never caller
    # supplied TypeScript: the factory compiles and lints them before the runner
    # receives a file.
    journey_payloads: list[dict] = Field(default_factory=list, max_length=20)
    base_url: str = ""
...
async def playwright_run(
    ...
    async with tenant_scoped_session(tenant_id) as session:
        ...
        edited_map = await _active_edited_map(session, artifact_id=artifact_id)
    # A direct journey is run through the same project/runner path as an
    # artifact case, but it is intentionally not materialised as a mutable
    # FactoryTestCaseRow.  That row is artifact-owned while a journey test id is
    # stable across re-crawls; storing it there would make the next artifact
    # silently steal the old artifact's test.  Compile it into this RUN's
    # ephemeral project instead.
    from ..services.script_factory import journey_compiler as _journey_compiler
    journey_compile = _journey_compiler.compile_top_n(
        body.journey_payloads, parametrize=True,
    ) if body.journey_payloads else {"results": [], "specs": {}}
    journey_results = list(journey_compile.get("results") or [])
    journey_refused = [r for r in journey_results if not r.get("compiled")]
    journey_lint_failed = [r for r in journey_results
                           if int(r.get("lint_errors") or 0) > 0]
    if journey_refused or journey_lint_failed:
        raise HTTPException(status_code=422, detail={
            "error": "one or more direct journeys are not safe to run",
            "compile_refusals": journey_refused,
            "lint_failures": journey_lint_failed,
        })
    journey_specs = dict(journey_compile.get("specs") or {})
    direct_ids = {str(r.get("test_id") or "") for r in journey_results
                  if str(r.get("test_id") or "")}
    if tcids:
        cases = [c for c in cases if (getattr(c, "test_id", "") or "") in tcids]
    elif cats:
        cases = [c for c in cases if (getattr(c, "type", "") or "").lower() in cats]
    case_ids = {str(getattr(c, "test_id", "") or "") for c in cases}
    collision = sorted(direct_ids & case_ids)
    if collision:
        raise HTTPException(status_code=409, detail={
            "error": "direct journey test id collides with an artifact case",
            "test_ids": collision,
        })
    if not cases and not journey_specs:
        raise HTTPException(status_code=404, detail="no matching active test cases to run")
    ...
    # The runner executes the same generated bytes linted above.  Adding these
    # after ``_configured_files`` is intentional: the helper owns artifact
    # cases, while direct journey specs belong only to this job and must never
    # be persisted or accidentally included in a later artifact export.
    files.update(journey_specs)
    env = {
        "NEXUS_ENDPOINT": _INGEST_BASE,
        "NEXUS_DIAGNOSTICS_ENDPOINT": f"{_INGEST_BASE}/api/v1/test-runs/diagnostics",
        "NEXUS_TOKEN": token or "",
    ...
    await _register_job(run_id, {
        ...
        "environment_id": environment_id,
        "target": base_url, "scripts": len(cases) + len(journey_specs), "exit_code": None, "output": "",
        "steps_completed": 0, "total_tests": len(cases) + len(journey_specs),
    })
