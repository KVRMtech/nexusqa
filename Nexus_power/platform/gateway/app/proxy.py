...
async def proxy_request(
    ...
    *,
    ...
    if auth_header.startswith("Bearer "):
        ...
        # Fallback: accept ?token= query param for SSE (EventSource cannot set headers)
    ...
    if token:
        ...
        logger.warning("gateway.missing_auth_token", extra={"path": full_path})
        raise HTTPException(
            status_code=401,
    ...
    if not is_health:
        # Support both sync (in-memory) and async (Redis) rate limiters
        allowed = rate_limiter.allow(tenant_id)
