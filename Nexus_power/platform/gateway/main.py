...
_in_memory_limiter = RateLimiter(config.rate_limit_per_minute)
...
class _DistributedRateLimiter:
    """Wrapper: use Redis limiter if connected, fall back to in-memory."""

    def __init__(self, redis_limiter: RedisRateLimiter, fallback: RateLimiter):
    ...
    async def allow(self, tenant_id: str):
        if self._redis.is_connected:
            return await self._redis.allow(tenant_id)
        return self._fallback.allow(tenant_id)


_rate_limiter = _DistributedRateLimiter(_redis_limiter, _in_memory_limiter)
...
async def _on_startup():
    ...
    logger.info(
        "gateway.rate_limiter: %s",
        "redis" if _redis_limiter.is_connected else "in-memory (fallback)",
    )
