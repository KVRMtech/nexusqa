...
class TestFrameExtractor:
    ...
    def test_init_defaults(self):
        ...
        fe = FrameExtractor()
        # Defaults updated post-OCR-tuning (architect P0 follow-up):
        #   threshold 0.05 → 0.03 (dHash on 9x8 — text-field changes
        #   were under the 5% threshold and being dropped)
        #   max_fps    1.0 → 2.0 (baseline 0.5s gap before adaptive
        #   slow-down kicks in)
        assert fe.frame_diff_threshold == 0.03
        assert fe.max_fps_extract == 2.0
...
class TestOCREngine:
    ...
    def test_load_stub_fallback(self, monkeypatch):
        # The production degradation guard refuses to fall back to the
        # stub OCR unless NEXUS_ALLOW_DEGRADED_MODE=true. The test is
        # specifically validating the stub path, so opt in here.
        monkeypatch.setenv("NEXUS_ALLOW_DEGRADED_MODE", "true")
        from app.vision import OCREngine
        ocr = OCREngine()
...
class TestVisualAnalyzer:
    ...
    def test_per_frame_llava_config_defaults_off(self, monkeypatch):
        """Per-frame LLaVA defaults off so existing prod behaviour is preserved."""
        monkeypatch.setenv("EYES_PER_FRAME_LLAVA", "false")
        monkeypatch.setenv("EYES_PER_FRAME_LLAVA_LIMIT", "50")
        from main import EyesConfig
        cfg = EyesConfig()
        assert cfg.per_frame_llava is False
        assert cfg.per_frame_llava_limit == 50

    def test_per_frame_llava_config_reads_env(self, monkeypatch):
        """``EYES_PER_FRAME_LLAVA`` and limit overrides flow through pydantic."""
        from main import EyesConfig
        monkeypatch.setenv("EYES_PER_FRAME_LLAVA", "true")
        monkeypatch.setenv("EYES_PER_FRAME_LLAVA_LIMIT", "12")
        cfg = EyesConfig()
        assert cfg.per_frame_llava is True
        assert cfg.per_frame_llava_limit == 12

    def test_per_frame_ocr_config_defaults_off(self, monkeypatch):
        """Per-frame OCR defaults off — CPU EasyOCR is too slow to run on
        every frame by default.  Operators opt in on GPU/cloud-OCR deployments."""
        monkeypatch.delenv("EYES_PER_FRAME_OCR", raising=False)
        from main import EyesConfig
        cfg = EyesConfig()
        assert cfg.per_frame_ocr_in_multi_frame_scenes is False

    def test_per_frame_ocr_config_reads_env(self, monkeypatch):
        from main import EyesConfig
        monkeypatch.setenv("EYES_PER_FRAME_OCR", "true")
        cfg = EyesConfig()
        assert cfg.per_frame_ocr_in_multi_frame_scenes is True

    def test_ocr_languages_default_is_english(self, monkeypatch):
        monkeypatch.delenv("EYES_OCR_LANGUAGES", raising=False)
        from main import EyesConfig
        cfg = EyesConfig()
        assert cfg.ocr_languages == ["en"]

    def test_ocr_languages_parses_comma_separated_env(self, monkeypatch):
        """``EYES_OCR_LANGUAGES=en,es,fr`` should yield ['en','es','fr']."""
        monkeypatch.setenv("EYES_OCR_LANGUAGES", "en, ES ,fr,")
        from main import EyesConfig
        cfg = EyesConfig()
        assert cfg.ocr_languages == ["en", "es", "fr"]

    def test_ocr_languages_accepts_single_language(self, monkeypatch):
        monkeypatch.setenv("EYES_OCR_LANGUAGES", "ja")
        from main import EyesConfig
        cfg = EyesConfig()
        assert cfg.ocr_languages == ["ja"]

    def test_ocr_languages_falls_back_to_english_on_empty(self, monkeypatch):
        """Empty / whitespace-only configuration must not crash EasyOCR startup."""
        monkeypatch.setenv("EYES_OCR_LANGUAGES", "  , ,")
        from main import EyesConfig
        cfg = EyesConfig()
        assert cfg.ocr_languages == ["en"]

    def test_memory_ceiling_config_defaults(self, monkeypatch):
        monkeypatch.delenv("EYES_MEMORY_CEILING_MB", raising=False)
        monkeypatch.delenv("EYES_AGGRESSIVE_FRAME_EVICTION", raising=False)
        from main import EyesConfig
        cfg = EyesConfig()
        assert cfg.memory_ceiling_mb == 2048.0
        assert cfg.aggressive_frame_buffer_eviction is True

    def test_memory_ceiling_config_reads_env(self, monkeypatch):
        monkeypatch.setenv("EYES_MEMORY_CEILING_MB", "4096")
        monkeypatch.setenv("EYES_AGGRESSIVE_FRAME_EVICTION", "false")
        from main import EyesConfig
        cfg = EyesConfig()
        assert cfg.memory_ceiling_mb == 4096.0
        assert cfg.aggressive_frame_buffer_eviction is False
    ...
    def test_model_field_normalizers_handle_bad_llm_shapes(self):
        """LLM scalar responses should not break FrameAnalysis validation."""
        from app.vision import _as_list, _table_dicts

        assert _as_list("") == []
        assert _as_list("null") == []
        assert _as_list({"rows": 1}) == [{"rows": 1}]
        assert _as_list('[{"element_type": "button"}]') == [{"element_type": "button"}]
        assert _table_dicts("") == []
        assert _table_dicts(["not-a-table", {"rows": 2}]) == [{"rows": 2}]
...
class TestMainModuleReexports:
    ...
    def test_frame_analysis_coerces_empty_table_string(self):
        from main import FrameAnalysis, ApplicationType
        fa = FrameAnalysis(
            frame_id="f-002",
            frame_index=0,
