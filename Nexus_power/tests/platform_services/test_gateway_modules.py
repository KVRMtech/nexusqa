...
class TestProxy:
    ...
    def _make_auth_header():
        ...
        token = pyjwt.encode(payload, cfg.nexus_jwt_secret, algorithm=cfg.jwt_algorithm)
    ...
    async def test_proxy_no_matching_route(self):
        ...
        mock_request.headers = self._make_auth_header()
    ...
    async def test_proxy_rate_limit_exceeded(self):
        ...
        # Exhaust rate limit for the test tenant
        rl.allow("t-test")
    ...
    @pytest.mark.asyncio
    async def test_proxy_missing_auth_non_public(self):
        """Should return 401 when no Authorization header on non-public path."""
        from app.proxy import proxy_request
        from app.config import GatewayConfig
        from app.rate_limiter import RateLimiter
        from fastapi import HTTPException
