...
| **2** Backups + DR | restore rehearsed, evidence intact | `scripts/verdict_pg_backup.sh --restore-drill` (alembic-head match + no-empty-restore over a seeded row) — runs in `qec-ci.yml`, asserts BACKUP_OK + RESTORE_DRILL_PASS | ✅ **proven** (real Postgres, both markers) + blocking in CI |
...
| **4** Scale (admission is a mutex) | multi-replica can't double-crawl | real-Lua shared-mutex test runs in `qec-ci.yml` (QEC_TEST_REDIS_URL); Helm `verdict.validateScaling` FAILS a render that scales out without redis admission + advisory-lock leader (proven in `deploy-validation.yml`) | ✅ **proven** (CI + render-guard) |
...
| **0** platform/api under test | the 300+ oracle/auditor tests run | `platform-api-tests` job (per-file isolation) in `ci.yml`; found + surfaced 7 real regressions from the efd0269 VM-sync (`docs/FINDINGS_PLATFORM_API_REGRESSIONS_2026-07-21.md`) | ✅ suite green in CI; ⚠️ **7 frozen-factory regressions await founder sign-off** |
