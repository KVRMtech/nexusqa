...
function MissionCard({ mission, personas, onClick }: { mission: MissionSummary; personas: Persona[]; onClick: () => void }) {
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className="group w-full text-left rounded-lg border border-gray-200 bg-white p-4 hover:border-gray-300 hover:bg-gray-50 transition-all"
    >
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          ...
          <h3 className="text-sm font-semibold text-[#0a2540] truncate group-hover:text-blue-300 transition-colors">
            {mission.title}
          </h3>
          {mission.description && (
      ...
      <div className="mt-3 flex items-center justify-between gap-4">
        <div className="flex items-center gap-3 text-xs text-slate-8000">
          {persona && (
      ...
      {mission.tags.length > 0 && (
        <div className="mt-2 flex flex-wrap gap-1">
          {mission.tags.slice(0, 3).map((tag) => (
            <span key={tag} className="rounded bg-gray-100 px-1.5 py-0.5 text-[10px] text-slate-400">
              {tag}
            </span>
          ))}
          {mission.tags.length > 3 && (
            <span className="text-[10px] text-slate-8000">+{mission.tags.length - 3}</span>
          )}
        </div>
      )}
    </button>
...
function CreateMissionModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={onClose}>
      ...
      >
        <h2 className="text-lg font-bold text-[#0a2540] mb-4">New Mission</h2>
        {artifactId && (
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input
              ...
              className="w-full rounded border border-gray-200 bg-gray-50 px-3 py-2 text-sm text-[#0a2540] placeholder-slate-400 focus:border-blue-500 focus:outline-none"
          ...
          <div>
            ...
            <select
              ...
              className="w-full rounded border border-gray-200 bg-gray-50 px-3 py-2 text-sm text-[#0a2540] focus:border-blue-500 focus:outline-none"
              required
            >
          ...
          <div>
            ...
            <textarea
              ...
              rows={2}
              className="w-full rounded border border-gray-200 bg-gray-50 px-3 py-2 text-sm text-[#0a2540] placeholder-slate-400 focus:border-blue-500 focus:outline-none resize-none"
          ...
          <div className="flex justify-end gap-3 pt-2">
            <button
              type="button"
              onClick={onClose}
              className="rounded px-4 py-2 text-sm text-slate-400 hover:text-[#0a2540] transition-colors"
            >
              Cancel
            </button>
            <button
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="relative flex-1 min-w-[200px] max-w-sm">
          <Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-8000" />
          <input
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            placeholder="Search missions…"
            className="w-full rounded-lg border border-gray-200 bg-gray-50 pl-9 pr-3 py-2 text-sm text-[#0a2540] placeholder-slate-400 focus:border-blue-500 focus:outline-none"
          />
        </div>
            ...
            <button
              ...
              className={clsx(
                ...
                statusFilter === key
                  ...
                  : 'text-slate-400 hover:text-[#0a2540] hover:bg-gray-100',
              )}
            >
              {label}
            </button>
      ...
      {!isLoadingList && totalMissions > filteredMissions.length && (
        <div className="text-center text-xs text-slate-8000">
