...
export default function DataForgePage() {
  const { data: configs, isLive } = useApiData(
    () => api.listDataForgeConfigs(),
    EMPTY_CONFIGS,
  );
  const { data: results } = useApiData(
    () => api.listDataForgeResults(),
    EMPTY_RESULTS,
  );
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <div className="stat-card p-4">
          <p className="text-[11px] font-medium text-slate-400 uppercase tracking-wider">Configurations</p>
          <p className="text-2xl font-bold text-[#0a2540] mt-1">{configs.length}</p>
        </div>
        <div className="stat-card p-4">
          <p className="text-[11px] font-medium text-slate-400 uppercase tracking-wider">Records Generated</p>
          <p className="text-2xl font-bold text-amber-400 mt-1">{totalRecords.toLocaleString()}</p>
        </div>
        <div className="stat-card p-4">
          <p className="text-[11px] font-medium text-slate-400 uppercase tracking-wider">Avg Coverage</p>
          <p className="text-2xl font-bold text-green-400 mt-1">{avgCoverage.toFixed(1)}%</p>
        </div>
        <div className="stat-card p-4">
          <p className="text-[11px] font-medium text-slate-400 uppercase tracking-wider">Generating Now</p>
            ...
            {configs.map((cfg) => (
              ...
              >
                <div className="flex items-center justify-between mb-1">
                  <h3 className="text-sm font-medium text-slate-700">{cfg.name}</h3>
                  <span className={clsx(
                ...
                <p className="text-[11px] text-slate-400">{cfg.product} • {cfg.jurisdiction} • {cfg.count.toLocaleString()} records</p>
              </button>
            ))}
          </div>
              ...
              return (
                <div className="card p-5 space-y-5">
                  <div className="flex items-center justify-between">
                    <div>
                      <h2 className="text-lg font-semibold text-[#0a2540]">{cfg.name}</h2>
                      <p className="text-xs text-slate-400 mt-0.5">{STRATEGY_DESCRIPTIONS[cfg.strategy]}</p>
                    </div>
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    <div className="rounded-lg bg-white/[0.03] p-3">
                      ...
                      <p className="text-sm text-slate-700 mt-0.5">{cfg.product}</p>
                    </div>
                    <div className="rounded-lg bg-white/[0.03] p-3">
                      <p className="text-[10px] text-slate-400 uppercase tracking-wider">Jurisdiction</p>
                      <p className="text-sm text-slate-700 mt-0.5">{cfg.jurisdiction}</p>
                    </div>
                    <div className="rounded-lg bg-white/[0.03] p-3">
                      <p className="text-[10px] text-slate-400 uppercase tracking-wider">Count</p>
                      <p className="text-sm text-slate-700 mt-0.5">{cfg.count.toLocaleString()} records</p>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="space-y-1.5">
                      {cfg.fields.map((f, i) => (
                        ...
                        >
                          <div className="flex h-6 w-6 items-center justify-center rounded bg-white/5 text-[10px] font-bold text-slate-400">
                            {i + 1}
                          </div>
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2">
                              <span className="text-xs font-mono text-amber-400">{f.name}</span>
                              <span className="text-[10px] text-slate-8000">({f.type})</span>
                            </div>
                            <p className="text-[10px] text-slate-400 truncate">{f.constraints}</p>
                          </div>
                          <button className="text-slate-8000 hover:text-red-400 transition-colors">
                  ...
                  <div className="rounded-lg bg-gradient-to-br from-amber-500/5 to-orange-500/5 p-4 ring-1 ring-amber-500/10">
                    ...
                    <div className="grid grid-cols-3 gap-3 text-center">
                      <div>
                        <p className="text-lg font-bold text-[#0a2540]">{cfg.fields.length}</p>
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-slate-400">Est. Coverage</p>
                      </div>
                      <div>
                        <p className="text-lg font-bold text-[#0a2540]">{cfg.count.toLocaleString()}</p>
              ...
              <div className="flex items-center gap-4 p-4">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    <h3 className="text-sm font-medium text-slate-700">{res.configName}</h3>
                  </div>
