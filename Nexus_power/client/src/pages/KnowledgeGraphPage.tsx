    ...
    <div className="space-y-4 animate-fade-in">
      ...
      {artifactSource && (
        <div className="flex items-center gap-3 px-4 py-2 bg-nexus-500/10 border border-nexus-500/30 rounded-lg">
          ...
          <span className="text-xs text-slate-600">Showing visual graph entities from <span className="text-nexus-400 font-medium">{artifactSource}</span></span>
          {artifactNodes.length > 0 && (
            <span className="text-[10px] text-slate-400 ml-auto">{artifactNodes.length} nodes, {artifactEdges.length} edges from artifact</span>
          )}
        </div>
      )}
          ...
          <button
            ...
            className={clsx(
              ...
              filters.size === 0 || filters.has(type)
                ? 'ring-white/20 text-slate-700'
                : 'ring-white/5 text-slate-8000 opacity-50',
            )}
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
        ...
        <div className="lg:col-span-3 card overflow-hidden">
          {/* Toolbar */}
          <div className="flex items-center justify-between px-4 py-2 bg-white/[0.02] border-b border-gray-200">
          ...
          {/* SVG Graph */}
          <div className="relative bg-[#f5f7fa]/80" style={{ height: 500 }}>
        ...
        <div className="space-y-4">
          ...
          <div className="card p-4">
            ...
            <div className="space-y-2">
              ...
              {nlResult && (
                <div className="rounded-lg bg-nexus-500/10 p-3 text-xs text-slate-600 animate-fade-in">
                  {nlResult}
                </div>
              )}

              <div className="space-y-1.5 mt-3">
                <p className="text-[10px] text-slate-8000">Try asking:</p>
                {NL_SUGGESTIONS.map((s, i) => (
                  <button
                    key={i}
                    onClick={() => { setNlQuery(s); }}
                    className="block w-full text-left text-[11px] text-slate-400 hover:text-nexus-400 transition-colors truncate"
                  >
                    "{s}"
                  </button>
                ))}
          ...
          {selectedNode ? (
            <div className="card p-4 animate-slide-in">
              <div className="flex items-center gap-2 mb-3">
                ...
                <h3 className="text-sm font-semibold text-[#0a2540]">{selectedNode.label}</h3>
              </div>
              <div className="space-y-2 text-xs">
                ...
                <div>
                  <p className="text-slate-400 mb-1">Related:</p>
                  {mergedEdges
                    ...
                    .map((e, i) => {
                      ...
                      return other ? (
                        <button
                          ...
                          className="flex w-full items-center gap-2 rounded-md px-2 py-1.5 text-xs text-slate-8000 hover:bg-white/[0.04] transition-colors"
                        >
                          <span className="h-2 w-2 rounded-full shrink-0" style={{ backgroundColor: other.color }} />
                          <span className="truncate">{other.label}</span>
                          <span className="text-[10px] text-slate-8000 ml-auto">{e.label}</span>
                        </button>
                      ) : null;
                    })}
                </div>
            ...
            <div className="card p-4 text-center">
              <Network className="h-8 w-8 text-slate-8000 mx-auto mb-2" />
          ...
          {/* Graph Stats */}
          <div className="card p-4">
            <h3 className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-3">Graph Statistics</h3>
            <div className="space-y-2">
              {Array.from(nodeTypes.entries()).map(([type, count]) => (
                <div key={type} className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <span className="h-2 w-2 rounded-full" style={{ backgroundColor: NODE_COLORS[type] || '#666' }} />
                    <span className="text-xs text-slate-8000">{type}</span>
                  </div>
              ...
              <div className="border-t border-gray-200 pt-2 mt-2 flex justify-between">
                ...
                <span className="text-xs font-bold text-[#0a2540]">{mergedNodes.length} nodes / {mergedEdges.length} edges</span>
              </div>
            </div>
          </div>
        </div>
