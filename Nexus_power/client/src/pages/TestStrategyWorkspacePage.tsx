...
function coverageStatusColor(s: string): string {
  switch (s) {
    ...
    default:        return 'text-slate-8000 bg-gray-500/20 border-gray-500/30';
  }
}
...
function EvidenceTraceCard({ citation }: { citation: EvidenceCitation }) {
  return (
    <div className="flex items-start gap-2 rounded border border-gray-200 bg-gray-50 px-2.5 py-1.5 text-[11px]">
      <span className="mt-0.5 text-slate-400">{modalityIcon(citation.source_modality)}</span>
      <div className="flex-1 min-w-0">
        <p className="text-slate-600 leading-relaxed line-clamp-2">"{citation.text}"</p>
...
function TestStepRow({ step }: { step: TestStep }) {
  return (
    <tr className="border-b border-gray-200/50 last:border-0">
      <td className="py-1.5 pr-2 text-[10px] font-mono text-slate-400 align-top w-6">{step.step_number}</td>
      <td className="py-1.5 pr-2 text-[11px] text-slate-600 align-top">{step.action}</td>
      <td className="py-1.5 pr-2 text-[11px] text-slate-8000 align-top">{step.input_data || '—'}</td>
      <td className="py-1.5 text-[11px] text-slate-8000 align-top">{step.expected_behavior}</td>
    </tr>
  );
}
  ...
  return (
    <div className="rounded-lg border border-gray-200 bg-gray-50 overflow-hidden">
      ...
      >
        ...
        <span className="text-[10px] font-mono text-slate-400">{tc.case_id}</span>
        <span className="text-sm text-slate-700 flex-1 line-clamp-1">{tc.title}</span>
      ...
      {isExpanded && (
        <div className="border-t border-gray-200 px-4 py-3 space-y-3">
          ...
          {tc.preconditions.length > 0 && (
            <div>
              <h5 className="text-[10px] font-semibold uppercase tracking-wider text-slate-400 mb-1">Preconditions</h5>
              <ul className="space-y-0.5">
                {tc.preconditions.map((pre, i) => (
                  <li key={i} className="text-[11px] text-slate-8000 flex items-start gap-1.5">
          ...
          {tc.steps.length > 0 && (
            <div>
              <h5 className="text-[10px] font-semibold uppercase tracking-wider text-slate-400 mb-1">Test Steps</h5>
              <table className="w-full">
                <thead>
                  <tr className="border-b border-gray-200">
                    <th className="text-left text-[10px] text-slate-400 font-medium pb-1 w-6">#</th>
                    <th className="text-left text-[10px] text-slate-400 font-medium pb-1">Action</th>
                    <th className="text-left text-[10px] text-slate-400 font-medium pb-1">Input</th>
                    <th className="text-left text-[10px] text-slate-400 font-medium pb-1">Expected</th>
                  </tr>
                </thead>
                <tbody>
          ...
          {tc.expected_result && (
            <div>
              <h5 className="text-[10px] font-semibold uppercase tracking-wider text-slate-400 mb-1">Expected Result</h5>
          ...
          {tc.tags.length > 0 && (
            <div className="flex items-center gap-1.5 flex-wrap">
              {tc.tags.map((tag) => (
                <span key={tag} className="rounded-full bg-gray-100 px-2 py-0.5 text-[10px] text-slate-8000 border border-gray-200">
                  {tag}
                </span>
              ))}
            </div>
...
function ScenarioSection({ scenario }: { scenario: TestScenario }) {
  ...
  return (
    <div className="rounded-xl border border-gray-200 bg-white/90 overflow-hidden">
      {/* Scenario Header */}
      <button
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          {scenario.description && (
            <p className="text-[11px] text-slate-400 mt-0.5 line-clamp-1">{scenario.description}</p>
          )}
        </div>
...
function CategorySection({ category, cases }: { category: string; cases: TestCase[] }) {
  ...
  return (
    <div className="rounded-xl border border-gray-200 bg-white/90 overflow-hidden">
      ...
      >
        {isOpen ? (
          <ChevronDown className="h-5 w-5 text-slate-8000 shrink-0" />
        ) : (
          <ChevronRight className="h-5 w-5 text-slate-8000 shrink-0" />
        )}
        <span className={clsx('inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-xs font-medium', cat.color)}>
          {cat.icon} {cat.label}
        </span>
        <div className="flex-1 min-w-0">
          <p className="text-[11px] text-slate-400">{cases.length} test case{cases.length !== 1 ? 's' : ''}</p>
        </div>
        <span className="rounded-full bg-gray-100 border border-gray-200 px-2 py-0.5 text-[10px] font-mono text-slate-8000">
          {cases.length}
        </span>
      </button>
    ...
    return (
      <div className="flex h-full items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-sm text-slate-8000">Loading test strategy...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="flex h-full items-center justify-center">
        <div className="flex flex-col items-center gap-4 max-w-md text-center">
          ...
          <h2 className="text-lg font-semibold text-slate-700">Test Architect is Analyzing...</h2>
          ...
          <p className="text-xs text-slate-400 animate-pulse">This may take 1-3 minutes on CPU...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="flex h-full items-center justify-center">
        <div className="flex flex-col items-center gap-4 max-w-md text-center">
          ...
          <h2 className="text-lg font-semibold text-slate-700">Test Strategy Generation Failed</h2>
          <p className="text-sm text-red-400">{error}</p>
          <div className="flex gap-3">
            <button
              onClick={() => navigate(-1)}
              className="rounded-lg bg-gray-100 px-4 py-2 text-sm text-slate-600 hover:bg-gray-100 transition-colors"
            >
              Go Back
            </button>
            <button
    ...
    <div className="flex h-full flex-col overflow-hidden bg-[#f5f7fa]">
      ...
      <div className="shrink-0 border-b border-gray-200 bg-white px-6 py-3">
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1 min-w-0">
            <h1 className="text-sm font-semibold text-slate-700 line-clamp-1">
          ...
          <button
            ...
            className="rounded-lg bg-gray-100 px-3 py-1.5 text-xs font-medium text-slate-600 hover:bg-gray-100 transition-colors flex items-center gap-1.5"
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="w-80 shrink-0 border-r border-gray-200 overflow-y-auto p-4 space-y-4">
          ...
          <div className="rounded-xl border border-gray-200 bg-white/90 p-4 space-y-3">
            ...
            {plan.objective && (
              <div>
                <span className="text-[10px] text-slate-400 uppercase">Objective</span>
                <p className="text-[11px] text-slate-600 mt-0.5">{plan.objective}</p>
              </div>
            )}
            {plan.scope && (
              <div>
                <span className="text-[10px] text-slate-400 uppercase">Scope</span>
                <p className="text-[11px] text-slate-600 mt-0.5">{plan.scope}</p>
              </div>
            )}
            <div>
              <span className="text-[10px] text-slate-400 uppercase">Approach</span>
              <p className="text-[11px] text-slate-600 mt-0.5 capitalize">{plan.approach}</p>
            </div>
          </div>
          ...
          <div className="rounded-xl border border-gray-200 bg-white/90 p-4 space-y-3">
            ...
            <div>
              <div className="flex items-center justify-between text-[11px] mb-1">
                <span className="text-slate-8000">Step Coverage</span>
            ...
            <div className="grid grid-cols-2 gap-2">
              <div className="rounded-lg bg-white border border-gray-200 p-2 text-center">
                <p className="text-lg font-bold text-slate-700">{stats.totalScenarios}</p>
                <p className="text-[10px] text-slate-400">Scenarios</p>
              </div>
              <div className="rounded-lg bg-white border border-gray-200 p-2 text-center">
                <p className="text-lg font-bold text-slate-700">{stats.totalCases}</p>
                <p className="text-[10px] text-slate-400">Test Cases</p>
              </div>
            </div>
                ...
                {[['P0_critical', stats.p0Count], ['P1_high', stats.p1Count], ['P2_medium', stats.p2Count], ['P3_low', stats.p3Count]].map(([key, count]) => {
                  ...
                  return (
                    <div key={key as string} className="flex items-center gap-2 text-[11px]">
                      ...
                      <span className="text-slate-8000 flex-1">{conf.label}</span>
            ...
            <div>
              ...
              <div className="mt-1 space-y-1">
                {[['happy_path', stats.happyPath], ['negative', stats.negative], ['boundary', stats.boundary], ['edge_case', stats.edgeCase]].map(([key, count]) => {
                  ...
                  return (
                    <div key={key as string} className="flex items-center gap-2 text-[11px]">
                      <span className="text-slate-400">{conf.icon}</span>
                      <span className="text-slate-8000 flex-1">{conf.label}</span>
                      <span className="font-mono font-semibold text-slate-600">{count as number}</span>
                    </div>
                  );
                })}
              </div>
          ...
          <div className="rounded-xl border border-gray-200 bg-white/90 p-4 space-y-2">
            ...
            <div className="space-y-1 text-[11px]">
              <div className="flex justify-between"><span className="text-slate-400">Model</span><span className="text-slate-600 font-mono">{prov.model_used}</span></div>
              <div className="flex justify-between"><span className="text-slate-400">Steps Analysed</span><span className="text-slate-600">{prov.workflow_steps_analysed}</span></div>
              <div className="flex justify-between"><span className="text-slate-400">Risks Considered</span><span className="text-slate-600">{prov.risks_considered}</span></div>
              <div className="flex justify-between"><span className="text-slate-400">Generation</span><span className="text-slate-600">{(prov.generation_time_ms / 1000).toFixed(1)}s</span></div>
              {strategy.cached ? (
                <>
                  <div className="flex justify-between"><span className="text-slate-400">Original Processing</span><span className="text-slate-600">{(strategy.processing_time_ms / 1000).toFixed(1)}s</span></div>
                  <div className="flex justify-between"><span className="text-slate-400">Cache Load</span><span className="text-emerald-400 font-mono">{strategy.cache_hit_ms != null ? `${strategy.cache_hit_ms.toFixed(0)}ms` : '—'}</span></div>
                </>
              ) : (
                <div className="flex justify-between"><span className="text-slate-400">Total Time</span><span className="text-slate-600">{(strategy.processing_time_ms / 1000).toFixed(1)}s</span></div>
              )}
              {prov.generated_at && (
                <div className="flex justify-between"><span className="text-slate-400">Generated</span><span className="text-slate-600">{new Date(prov.generated_at).toLocaleString()}</span></div>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-4">
          <div className="flex items-center gap-2 mb-2">
            ...
            <div className="ml-auto flex items-center gap-1 rounded-lg bg-white border border-gray-200 p-0.5">
              <button
                ...
                className={clsx(
                  ...
                  viewMode === 'category'
                    ...
                    : 'text-slate-8000 hover:text-slate-700'
                )}
              >
                By Category
              </button>
          ...
          <div className="space-y-2">
            <div className="relative">
              ...
              <input
                ...
                className="w-full rounded-lg border border-gray-200 bg-white pl-9 pr-8 py-2 text-xs text-slate-700 placeholder-slate-400 focus:border-nexus-500/50 focus:outline-none focus:ring-1 focus:ring-nexus-500/30"
              />
              {searchQuery && (
                ...
                return (
                  <button
                    ...
                    className={clsx(
                      ...
                      isActive ? cfg.color : 'text-slate-400 bg-gray-50 border-gray-200 hover:border-gray-200/50'
                    )}
                  >
                    {cfg.icon} {cfg.label}
              ...
              return (
                <>
                  {searchQuery || activeFilters.size > 0 ? (
                    <p className="text-[10px] text-slate-400">{totalCases} test case{totalCases !== 1 ? 's' : ''} match</p>
                  ) : null}
                  {filtered.map((sc) => (
                    <ScenarioSection key={sc.scenario_id} scenario={sc} />
                  ))}
                  {filtered.length === 0 && (
                    <p className="text-sm text-slate-400 text-center py-8">No test cases match your search.</p>
                  )}
                </>
              );
            } else {
                ...
                <>
                  ...
                  {filtered.length > showMax && (
                    <button
                      ...
                      className="w-full rounded-lg border border-gray-200 bg-gray-50 py-2 text-xs text-slate-8000 hover:text-slate-700 hover:bg-white transition-colors"
                    >
...
function TraceRow({ entry }: { entry: TraceabilityEntry }) {
  ...
  return (
    <div className="rounded-lg border border-gray-200 bg-gray-50 overflow-hidden">
      ...
      >
        <span className="flex items-center justify-center h-5 w-5 rounded bg-gray-100 text-[10px] font-mono text-slate-8000 shrink-0">
          {entry.workflow_step_number}
        </span>
        <span className="text-[11px] text-slate-600 flex-1 line-clamp-1">{entry.requirement}</span>
        ...
        <div className="border-t border-gray-200 px-3 py-2 space-y-1">
          {entry.test_case_ids.map((id) => (
            <div key={id} className="flex items-center gap-1.5 text-[10px]">
              ...
              <span className="text-slate-8000 font-mono">{id}</span>
            </div>
          ))}
