        ...
        return (
          <button
            ...
            className={clsx(
              ...
              status === 'skipped' && 'border-slate-700 bg-slate-800/50 text-slate-500',
              status === 'pending' && 'border-slate-700 bg-slate-800/50 text-slate-400',
            )}
          >
  ...
  return (
    <div className="rounded-lg border border-slate-700 bg-slate-800/50 overflow-hidden">
        ...
        <div className="flex items-center gap-2">
          ...
          {stage.status === 'active' && (
            <>
              ...
              <button
                onClick={onAdvance}
                className="flex items-center gap-1 rounded border border-slate-600 px-3 py-1.5 text-xs font-medium text-slate-300 hover:bg-slate-700 transition-colors"
              >
      ...
      {(stage.started_at || stage.duration_seconds > 0) && (
        <div className="border-b border-slate-700/50 px-4 py-2 flex items-center gap-4 text-xs text-slate-500">
          {stage.started_at && (
            ...
            {stage.engine_calls.map((call, i) => (
              <div
                ...
                className={clsx(
                  ...
                  call.status === 'skipped' && 'bg-slate-700 text-slate-500',
                )}
              >
      ...
      {stage.outputs && Object.keys(stage.outputs).length > 0 && (
        <div className="border-b border-slate-700/50 px-4 py-3">
          ...
          <pre className="rounded bg-slate-900 p-3 text-xs text-slate-300 overflow-x-auto max-h-48">
            ...
            {stageArtifacts.map((artifact) => (
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-white font-medium">{artifact.name}</span>
                  <span className="text-[10px] text-slate-500 uppercase">{artifact.artifact_type}</span>
                </div>
                {artifact.item_count > 0 && (
                  <span className="text-[10px] text-slate-500">{artifact.item_count} items</span>
                )}
              </div>
            ))}
          </div>
...
function ChatPanel({
  ...
  return (
    <div className="flex flex-col rounded-lg border border-slate-700 bg-slate-800/50 overflow-hidden h-[500px]">
      {/* Chat Header */}
      <div className="border-b border-slate-700 px-4 py-2 flex items-center gap-2">
        <MessageSquare className="h-4 w-4 text-blue-400" />
        <span className="text-xs font-medium text-white">Mission Chat</span>
        <span className="text-xs text-slate-500">— {personaIcon} {personaName}</span>
      </div>
      ...
      {/* Input */}
      <form onSubmit={handleSubmit} className="border-t border-slate-700 px-3 py-2 flex gap-2">
        <input
          value={input}
          onChange={(e) => setInput(e.target.value)}
          disabled={isSending}
          placeholder="Type a message…"
          className="flex-1 rounded border border-slate-600 bg-slate-900 px-3 py-2 text-xs text-white placeholder-slate-500 focus:border-blue-500 focus:outline-none disabled:opacity-50"
        />
        <button
          type="submit"
...
function ProgressBar({ value, className }: { value: number; className?: string }) {
  return (
    <div className={clsx('h-2 w-full rounded-full bg-slate-700', className)}>
      <div
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-start justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="rounded p-1.5 text-slate-500 hover:text-red-400 hover:bg-slate-700 transition-colors"
      ...
      <div className="space-y-1">
        <div className="flex items-center justify-between text-xs">
          ...
          <span className="text-white font-medium">{Math.round(mission.progress_pct)}%</span>
        </div>
        ...
        <div className="rounded-lg border border-slate-700 bg-slate-800/50 p-4">
          ...
          {mission.objective && (
            <div>
              ...
              <p className="text-sm text-slate-300">{mission.objective}</p>
            </div>
          )}
        </div>
      )}
      ...
      {showDeleteConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={() => setShowDeleteConfirm(false)}>
          ...
          >
            <h3 className="text-sm font-bold text-white mb-2">Delete Mission?</h3>
