      ...
      {stages.map(([stageType, cfg]) => {
        ...
        return (
          <div
            ...
            className="rounded bg-gray-100 px-1.5 py-0.5 text-[10px] text-slate-400"
          >
...
function PersonaCard({
  ...
  return (
    <div className="group rounded-lg border border-gray-200 bg-white p-4 hover:border-gray-300 hover:bg-gray-50 transition-all">
      <div className="flex items-start gap-3">
        <div className="flex-shrink-0 flex items-center justify-center h-10 w-10 rounded-lg bg-gray-100 text-lg">
          {persona.avatar_icon || '🤖'}
        </div>
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <h3 className="text-sm font-semibold text-[#0a2540] truncate">{persona.name}</h3>
            {persona.is_system && (
      ...
      {persona.capabilities.length > 0 && (
        <div className="mt-3 flex flex-wrap gap-1">
          {persona.capabilities.slice(0, 4).map((cap) => (
            <span
              key={cap}
              className="rounded bg-gray-100/70 px-1.5 py-0.5 text-[10px] text-slate-600"
            >
              {cap}
            </span>
          ))}
          {persona.capabilities.length > 4 && (
            <span className="text-[10px] text-slate-8000">
      ...
      <div className="mt-3 flex items-center justify-between pt-2 border-t border-gray-200">
        <button
          onClick={onSelect}
          className="text-[11px] text-slate-400 hover:text-[#0a2540] transition-colors flex items-center gap-1"
        >
...
function PersonaDetailDrawer({
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end bg-black/50" onClick={onClose}>
      <div
        className="w-full max-w-md bg-white border-l border-gray-200 overflow-y-auto"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="p-6 space-y-5">
          {/* Header */}
          <div className="flex items-center gap-3">
            <div className="flex items-center justify-center h-12 w-12 rounded-lg bg-gray-100 text-xl">
              {persona.avatar_icon || '🤖'}
            </div>
            <div>
              <h2 className="text-base font-bold text-[#0a2540]">{persona.name}</h2>
              <p className="text-xs text-slate-8000">{persona.slug}</p>
            </div>
          </div>
          ...
          {persona.system_prompt && (
            <div>
              ...
              <pre className="rounded bg-gray-50 p-3 text-xs text-slate-600 whitespace-pre-wrap max-h-40 overflow-y-auto">
                {persona.system_prompt}
              </pre>
            </div>
          )}
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-1.5">
                {persona.capabilities.map((cap) => (
                  <span key={cap} className="rounded bg-gray-100 px-2 py-0.5 text-xs text-slate-600">
                    {cap}
                  </span>
                ))}
              </div>
            ...
            <div>
              ...
              <div className="space-y-2">
                {Object.entries(persona.stage_config).map(([stage, cfg]) => {
                  ...
                  return (
                    <div key={stage} className="rounded border border-gray-200 bg-gray-50 p-2.5">
                      ...
                      <div className="flex flex-wrap gap-1">
                        {cfg.engines.map((engine: string) => (
                          <span key={engine} className="rounded bg-gray-100 px-1.5 py-0.5 text-[10px] text-slate-400">
                            {engine}
                          </span>
                        ))}
                      </div>
          ...
          {/* Meta */}
          <div className="border-t border-gray-200 pt-3 space-y-1 text-xs text-slate-8000">
          ...
          <button
            onClick={onClose}
            className="w-full rounded border border-gray-200 px-4 py-2 text-xs text-slate-400 hover:text-[#0a2540] hover:bg-gray-50 transition-colors"
          >
            Close
          </button>
        </div>
      ...
      {!isLoading && systemPersonas.length > 0 && (
        <div>
          <div className="flex items-center gap-2 mb-3">
            ...
            <h2 className="text-sm font-semibold text-[#0a2540]">System Personas</h2>
            <span className="text-xs text-slate-8000">({systemPersonas.length})</span>
          </div>
      ...
      {!isLoading && customPersonas.length > 0 && (
        <div>
          <div className="flex items-center gap-2 mb-3">
            ...
            <h2 className="text-sm font-semibold text-[#0a2540]">Custom Personas</h2>
            <span className="text-xs text-slate-8000">({customPersonas.length})</span>
          </div>
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
