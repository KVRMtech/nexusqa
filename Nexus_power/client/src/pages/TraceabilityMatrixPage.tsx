    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-3">
        <div className="stat-card p-4">
          <p className="text-[11px] font-medium text-slate-400 uppercase tracking-wider">Total Rules</p>
          <p className="text-2xl font-bold text-[#0a2540] mt-1">{traces.length}</p>
        </div>
        <button
        ...
        >
          <p className="text-[11px] font-medium text-slate-400 uppercase tracking-wider">Full Coverage</p>
        ...
        >
          <p className="text-[11px] font-medium text-slate-400 uppercase tracking-wider">Partial</p>
        ...
        >
          <p className="text-[11px] font-medium text-slate-400 uppercase tracking-wider">No Coverage</p>
          <p className="text-2xl font-bold text-red-400 mt-1">{noCoverage}</p>
        </button>
        <div className="stat-card p-4">
          <p className="text-[11px] font-medium text-slate-400 uppercase tracking-wider">Failing Tests</p>
          <p className="text-2xl font-bold text-red-400 mt-1">{failingTests}</p>
          <p className="text-[11px] text-slate-400">of {totalTests} total</p>
        </div>
      </div>
            ...
            <div key={row.id} className="card overflow-hidden">
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1.5 text-xs text-slate-8000 flex-wrap">
                    <Radio className="h-3 w-3 text-pink-400" />
                    <span className="font-medium text-slate-600 truncate max-w-[120px]">{row.speaker}</span>
                    <ArrowRight className="h-3 w-3 text-slate-8000" />
                  ...
                  <p className="text-sm text-slate-700 mt-1 truncate">{row.ruleTitle}</p>
                  <p className="text-[10px] text-slate-8000 mt-0.5">{row.session} • {row.sessionDate}</p>
                </div>
                ...
                <div className="text-right shrink-0">
                  ...
                  <p className="text-[10px] text-slate-8000">confidence</p>
                </div>
              ...
              {isExpanded && (
                <div className="border-t border-gray-200 p-4 animate-slide-in">
                  {row.tests.length === 0 ? (
                    <div className="text-center py-6">
                      <AlertTriangle className="h-8 w-8 text-red-400/50 mx-auto mb-2" />
                      <p className="text-sm text-slate-8000">No tests linked to this rule</p>
                        ...
                        <tbody>
                          {row.tests.map((tc) => (
                            <tr key={tc.testId} className="border-b border-white/[0.03] last:border-0">
                              ...
                              <td className="py-2 pr-2 text-xs text-slate-600">{tc.title}</td>
                              ...
                              <td className="py-2 pr-2">
                                <div className="flex items-center gap-1.5 text-xs">
                                  ...
                                  <span className={clsx(
                                    ...
                                    tc.lastResult === 'not-run' && 'text-slate-400',
                                  )}>
                                    {tc.lastResult}
                                  </span>
                                </div>
                              ...
                              <td className="py-2 text-right">
                                <div className="flex items-center justify-end gap-2">
                                  ...
                                  <span className="text-xs text-slate-8000 w-8 text-right">{tc.passRate}%</span>
                                </div>
                              </td>
                            </tr>
                          ))}
                  ...
                  {row.jiraTickets.length > 0 && (
                    <div className="mt-3 pt-3 border-t border-gray-200 flex items-center gap-2">
                      ...
                      <span className="text-xs text-slate-400">Linked Jira:</span>
