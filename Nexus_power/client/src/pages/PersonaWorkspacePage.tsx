...
function modalityBadgeColor(mod: string): string {
  switch (mod) {
    ...
    case 'inferred':   return 'bg-gray-500/20 text-slate-500 border-gray-500/30';
    default:           return 'bg-gray-500/20 text-slate-500 border-gray-500/30';
  }
}
...
function severityColor(s: string): string {
  switch (s) {
    ...
    default:         return 'text-slate-500 bg-gray-500/20 border-gray-500/30';
  }
}
  ...
  return (
    <div className="space-y-1.5">
      ...
      >
        ...
        <span className="ml-auto text-[10px] font-mono text-slate-500">{citations.length}</span>
      </button>
      {open && (
  ...
  return (
    <div className="rounded-lg border border-gray-200 bg-white p-3 space-y-2">
      <div className="flex items-start gap-2">
        <span className="shrink-0 text-[10px] font-mono font-bold text-slate-400 bg-gray-100 rounded px-1.5 py-0.5">
          E{index + 1}
        </span>
        <p className="text-xs text-slate-600 leading-relaxed flex-1">{citation.text}</p>
      </div>
      <div className="flex items-center gap-2 flex-wrap">
        ...
        {citation.timestamp_range && (
          <span className="text-[10px] text-slate-400 font-mono">{citation.timestamp_range}</span>
        )}
  ...
  return (
    <div className="relative">
      ...
      <div
        ...
        className="relative flex items-start gap-3 p-3 rounded-lg border border-gray-200 bg-gray-50 hover:bg-white hover:border-[#2670a3]/30 transition-all cursor-pointer group"
      >
        {/* Step number circle */}
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <h4 className="text-sm font-semibold text-[#0a2540] group-hover:text-nexus-300">{step.name}</h4>
      ...
      {/* Expanded details */}
      {expanded && (
        <div className="ml-13 mt-2 mb-4 space-y-2 pl-4 border-l border-gray-100">
          {step.decisions.length > 0 && (
            <div>
              <h5 className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider mb-1">Decisions</h5>
          ...
          {step.evidence.length > 0 && (
            <div>
              <h5 className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider mb-1">Evidence ({step.evidence.length})</h5>
              <div className="space-y-1.5">
      ...
      <div className="flex items-start gap-2">
        ...
        <div>
          ...
          <div className="flex items-center gap-2 mt-1">
            ...
            <span className="text-[10px] text-slate-400">· {risk.evidence.length} citation{risk.evidence.length !== 1 ? 's' : ''}</span>
          </div>
        </div>
      </div>
    </div>
  ...
  return (
    <div className="rounded-xl border border-gray-200 bg-white p-4 space-y-3">
      <div className="flex items-center gap-2">
        ...
        <h3 className="text-xs font-semibold text-[#0a2540] uppercase tracking-wider">Grounding Contract</h3>
      </div>

      {/* Stats row */}
      <div className="grid grid-cols-3 gap-3">
        <div className="text-center">
          <div className="text-lg font-bold text-[#0a2540]">{contract.total_evidence_count}</div>
          <div className="text-[10px] text-slate-400 uppercase">Citations</div>
        </div>
        <div className="text-center">
        ...
        <div className="text-center">
          <div className="text-lg font-bold text-[#0a2540]">{contract.open_questions.length}</div>
          <div className="text-[10px] text-slate-400 uppercase">Open Questions</div>
        </div>
      </div>
      ...
      {total > 0 && (
        <div>
          <div className="text-[10px] text-slate-400 mb-1 uppercase tracking-wider">Evidence Sources</div>
          ...
          <div className="flex gap-3 mt-1.5 flex-wrap">
            ...
            {dist.inferred > 0 && <span className="text-[10px] text-slate-500">● Inferred ({dist.inferred})</span>}
          </div>
        </div>
      )}
      ...
      {contract.open_questions.length > 0 && (
        <div>
          <div className="text-[10px] text-slate-400 mb-1.5 uppercase tracking-wider">Open Questions</div>
          <ul className="space-y-1">
...
export default function PersonaWorkspacePage() {
    ...
    return (
      <div className="flex h-full items-center justify-center bg-[#f5f7fa]">
        <div className="flex flex-col items-center gap-4 max-w-md text-center">
          ...
          <h2 className="text-xl font-bold text-[#0a2540]">Generating Process Oracle</h2>
    ...
    return (
      <div className="flex h-full items-center justify-center bg-[#f5f7fa]">
        <div className="flex flex-col items-center gap-4 max-w-md text-center">
          ...
          <h2 className="text-xl font-bold text-[#0a2540]">Generation Failed</h2>
          <p className="text-sm text-red-400">{error}</p>
          <div className="flex gap-3">
            <button
              onClick={() => navigate(-1)}
              className="px-4 py-2 text-sm text-slate-500 border border-gray-200 rounded-lg hover:bg-gray-100 transition-colors"
            >
              Go Back
            </button>
            <button
    ...
    return (
      <div className="flex h-full items-center justify-center bg-[#f5f7fa]">
        <div className="flex flex-col items-center gap-4 max-w-md text-center">
          ...
          <h2 className="text-xl font-bold text-[#0a2540]">Persona Saved</h2>
          <p className="text-sm text-slate-500">
            <strong className="text-[#0a2540]">{draft?.persona.name}</strong> has been saved successfully.
          </p>
          <div className="flex gap-3">
            <button
  ...
  return (
    <div className="flex flex-col h-full bg-[#f5f7fa] overflow-hidden">
      {/* ── Top Bar ────────────────────────────────────────── */}
      <div className="shrink-0 border-b border-gray-200 bg-white/95 backdrop-blur-xl px-6 py-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <button
              onClick={() => navigate(`/sessions/${sessionId}/canonical`)}
              className="text-slate-500 hover:text-[#0a2540] transition-colors"
            >
          ...
          <div className="flex items-center gap-3">
            ...
            {isCached && (
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-medium text-slate-500 border border-gray-200 hover:bg-gray-100 hover:text-[#0a2540] transition-all"
              >
      ...
      <div className="flex flex-1 overflow-hidden">
          ...
          <div className="p-4 space-y-4">
            ...
            <div className="rounded-xl border border-gray-200 bg-white p-4 space-y-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-sm font-bold text-[#0a2540]">{persona.name}</h2>
                  <p className="text-xs text-slate-500">{persona.specialty_domains.join(' · ')}</p>
                </div>
              </div>
            ...
            <div className="rounded-xl border border-gray-200 bg-white p-4 space-y-3">
              ...
              <div className="space-y-2">
                {dm.actors.length > 0 && (
                  <div className="p-2 rounded-lg bg-gray-100/80">
                    ...
                    <div className="space-y-0.5">
                      {dm.actors.map((a, i) => (
                        <div key={i} className="text-xs text-slate-600 truncate" title={a.role}>{a.name}{a.role && a.role !== a.name ? <span className="text-slate-400"> — {a.role}</span> : null}</div>
                      ))}
                    </div>
                  </div>
                )}
                {dm.systems.length > 0 && (
                  <div className="p-2 rounded-lg bg-gray-100/80">
                    ...
                    <div className="space-y-0.5">
                      {dm.systems.map((s, i) => (
                        <div key={i} className="text-xs text-slate-600 truncate" title={s.purpose}>{s.name}</div>
                      ))}
                    </div>
                  </div>
                )}
                    ...
                    <div className="space-y-0.5">
                      {dm.workflows.sort((a, b) => a.step_number - b.step_number).map((w, i) => (
                        <div key={i} className="text-xs text-slate-600 truncate">
                          <span className="text-slate-400 font-mono">{w.step_number}.</span> {w.name}
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  <div className="p-2 rounded-lg bg-gray-100/80">
                    ...
                    <div className="space-y-0.5">
                      {dm.decisions.map((d, i) => (
                        <div key={i} className="text-xs text-slate-600 truncate">{d}</div>
                      ))}
                    </div>
                  </div>
                )}
              ...
              {dm.entities.length > 0 && (
                <div>
                  <div className="text-[10px] text-slate-400 mb-1">Key Entities</div>
                  <div className="flex flex-wrap gap-1">
                    {dm.entities.slice(0, 12).map((e, i) => (
                      <span key={i} className="text-[10px] px-1.5 py-0.5 rounded bg-gray-100 text-slate-500">{e}</span>
                    ))}
                    {dm.entities.length > 12 && (
                      <span className="text-[10px] text-slate-500">+{dm.entities.length - 12} more</span>
                    )}
                  </div>
                </div>
              )}
            ...
            <div className="rounded-xl border border-gray-200 bg-white p-4">
              ...
              <pre className="text-[10px] text-slate-500 whitespace-pre-wrap font-mono leading-relaxed max-h-40 overflow-y-auto custom-scrollbar">
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar">
          <div className="p-6 space-y-6">
            {/* Workflow steps */}
            <div>
              <h2 className="text-sm font-bold text-[#0a2540] uppercase tracking-wider mb-4 flex items-center gap-2">
                    ...
                    <div className="space-y-2">
                      {dm.actors.map((actor, i) => (
                        <div key={i} className="rounded-lg border border-gray-200 bg-gray-50 p-3">
                          <div className="text-sm font-semibold text-[#0a2540]">{actor.name}</div>
                          <div className="text-xs text-slate-500 mt-0.5">{actor.role}</div>
                          <div className="text-[10px] text-slate-500 mt-1">{actor.evidence.length} citation{actor.evidence.length !== 1 ? 's' : ''}</div>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      {dm.systems.map((sys, i) => (
                        <div key={i} className="rounded-lg border border-gray-200 bg-gray-50 p-3">
                          <div className="text-sm font-semibold text-[#0a2540]">{sys.name}</div>
                          <div className="text-xs text-slate-500 mt-0.5">{sys.purpose}</div>
                          <div className="text-[10px] text-slate-500 mt-1">{sys.evidence.length} citation{sys.evidence.length !== 1 ? 's' : ''}</div>
                        </div>
                      ))}
                    </div>
                  </div>
            ...
            {dm.unknowns.length > 0 && (
              <div>
                <h3 className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-3 flex items-center gap-1.5">
                  <HelpCircle className="h-3.5 w-3.5 text-slate-500" /> Unknowns ({dm.unknowns.length})
                </h3>
                <div className="space-y-1.5">
                  {dm.unknowns.map((u, i) => (
                    <div key={i} className="text-xs text-slate-500 flex items-start gap-2 p-2 rounded-lg bg-gray-50">
        ...
        <div className="w-80 shrink-0 border-l border-gray-200 overflow-y-auto custom-scrollbar">
          <div className="p-4 space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="text-xs font-bold text-slate-400 uppercase tracking-wider flex items-center gap-1.5">
                <FileText className="h-3.5 w-3.5 text-nexus-400" /> Evidence Rail
              </h2>
              <span className="text-[10px] text-slate-500 font-mono">{allEvidence.length} total</span>
            </div>
            ...
            <div className="rounded-xl border border-gray-200 bg-white p-3 space-y-1.5">
              <h4 className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider">Provenance</h4>
              <div className="space-y-1 text-[10px] text-slate-500">
                <div className="flex justify-between">
                  <span>Artifact</span>
                  <span className="font-mono text-slate-400">{provenance.artifact_id.slice(0, 8)}...</span>
                </div>
                <div className="flex justify-between">
                ...
                <div className="flex justify-between">
                  <span>Model</span>
                  <span className="font-mono text-slate-400">{provenance.model_used}</span>
                </div>
                <div className="flex justify-between">
                  <span>Backend</span>
                  <span className="font-mono text-slate-400">{provenance.model_backend}</span>
                </div>
                <div className="flex justify-between">
                  <span>Generated</span>
