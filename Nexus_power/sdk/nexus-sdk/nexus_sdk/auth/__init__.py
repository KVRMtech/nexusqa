...
class NexusUser:
    ...
    tenant_id: str
    email: str
    role: str  # admin, manager, viewer, api, platform-admin
    permissions: list[str]
    name: Optional[str] = None

    @property
    def roles(self) -> list[str]:
        """List form of the role field, for code paths that expect a
        multi-role list (e.g. `"platform-admin" not in user.roles`).
        Workflow / DLQ admin checks were written against a multi-role
        identity model; this adapter keeps them working with the
        current single-role JWT claim without rewriting every call site."""
