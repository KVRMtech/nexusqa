...
def production_guard(component: str, available: bool) -> None:
    """
    Block degraded/stub fallback unless explicitly allowed.

    Call this after attempting to initialize a critical dependency
    (LLM, Playwright, Whisper, OCR, etc.). When ``available`` is False
    and degraded mode is not explicitly allowed, the service will
    refuse to start rather than silently serving stub responses.

    To opt in to degraded local development, set
    ...
    allow_degraded = os.getenv("NEXUS_ALLOW_DEGRADED_MODE", "false").lower() == "true"

    if not allow_degraded:
        raise RuntimeError(
            f"[NEXUS DEGRADATION GUARD] {component} failed to initialize. "
            f"Stub/degraded fallback is disabled by default. "
            f"Ensure the required dependency is installed and accessible, "
            f"or set NEXUS_ALLOW_DEGRADED_MODE=true to opt into degraded local mode."
        )
