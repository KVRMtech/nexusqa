...
class Database:
    ...
    async def tenant_session(self, tenant_id: str) -> AsyncGenerator[AsyncSession, None]:
        """
        Provide a transactional session scoped to a specific tenant.

        Sets the PostgreSQL session variable `nexus.current_tenant_id`
        so that Row-Level Security (RLS) policies can enforce tenant isolation
        at the database level.

        Usage:
            async with db.tenant_session("tenant-123") as session:
                result = await session.execute(select(SessionRow))
                # RLS ensures only tenant-123 rows are returned
        """
        if not self._session_factory:
            raise RuntimeError("Database not connected. Call connect() first.")
        async with self._session_factory() as session:
            try:
                ...
                # asyncpg raises ProgrammingError for "SET LOCAL var = $1",
                # so use set_config() which supports bound parameters and
                # behaves identically to SET LOCAL when is_local=true.
