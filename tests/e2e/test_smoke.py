...
def auth_token(client: httpx.Client):
    """Try several common default passwords to obtain a JWT."""
    for creds in [
...
class TestPlatformAPI:
    ...
    def test_insights_kpis(self, client):
        r = client.get(
    ...
    def test_admin_engines(self, client):
        r = client.get(url("platform-api", "/api/v1/admin/engines"))
        assert r.status_code == 200, r.text

    def test_admin_resources(self, client):
        r = client.get(url("platform-api", "/api/v1/admin/resources"))
        assert r.status_code in (200, 500), r.text
