...
class BackboneEngine(NexusEngine):
    ...
    async def on_startup(self):
        ...
        # ── Production guard: refuse to run with in-memory stores ──
        ...
        if _nexus_env == "production" and (using_inmemory_vector or using_inmemory_graph):
            stores = []
            ...
            logger.critical(
                "backbone: REFUSING to start in production with in-memory %s. "
                "Configure Milvus and Neo4j for production deployments.",
                " and ".join(stores),
            )
            import sys
            sys.exit(1)
