    ...
    async def probe_tier_health(self, engine_name: str) -> dict[str, Any]:
        ...
        for tier_num in (1, 2, 3):
            ...
            if provider_env in _CLOUD_PROVIDERS:
                # For cloud providers, check for per-engine tier key first,
                # then fall back to global provider key
                ...
                has_key = bool(per_engine_val or global_val)
                used_var = per_engine_key if per_engine_val else global_key
                results[tier_key]["healthy"] = has_key
                results[tier_key]["check"] = f"API key {'present' if has_key else 'MISSING'} ({used_var})"
