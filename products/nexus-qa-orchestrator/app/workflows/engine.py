...
class ChainEngine:
            ...
            if instance.status not in (
                ...
                if chain.chain_id == "nexus.canonical-processing":
                    ...
                    if cqg_stage and cqg_stage.output:
                        ...
                        gate_passed = quality_result.get("passed", True)
                        ...
                        # Stage missing/empty — advisory note, mark pass
                        quality_result = None
                        self._timeline(
                            instance,
                            "canonical_quality_gate_result",
                            "Quality gate (canonical): stage output missing — defaulting to pass",
                        )
                else:
    ...
    async def _poll_for_result(
        ...
        _STALL_WARN_SECS = 300.0   # warn after 5 min same sub-stage
        _STALL_DEGRADE_SECS = 720.0  # degrade after 12 min same sub-stage
        _stall_warned = False
        ...
        while time.monotonic() - start < poll_cfg.max_poll_seconds:
            ...
            if stall_secs >= _STALL_DEGRADE_SECS:
                logger.error(
                    ...
                    "stalled for %.0fs — consider intervention",
                    stage_def.stage_id, stage_def.engine, job_id,
    ...
    async def _update_canonical_artifact_status(
        ...
        try:
            ...
            if quality_result:
                ...
                # No quality result available — mark as pass (advisory)
                payload = {
