...
async def startup():
    global http_client, chain_engine
...
async def start_workflow(
    ...
    background_tasks.add_task(
        chain_engine.execute, instance.workflow_id, chain
    )
...
async def start_canonical_processing(
    ...
    if cached:
        ...
        await _update_session_status(tenant_id, session_id, "completed")
        logger.info(
    ...
    try:
        if workflow_store._redis:
            ...
            fp_lock_acquired = await workflow_store._redis.set(
                fp_lock_key, session_id, nx=True, ex=7200  # 2 hour TTL
            )
